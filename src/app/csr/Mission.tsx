  ...
  [
    {
      ...
      project:
        "Provided the villagers from the <br/> two towns with a state-of-the-<br/>art ambulance and garbage van",
  ...
  [
    {
      ...
      project:
        "Empowering communities and </br> spreading awareness around <br/> nutrition-rich food through <br/>competitions, drama, and<br/> music.",
  ...
  return (
    <>
      <div className="flex flex-col items-center justify-center bg-white p-4">
        <h1 className="font-playfair text-[35px] font-semibold leading-[58px] tracking-tight text-[#2067CA] bg-white mb-4 text-center">
          Pragati Mission
        </h1>

        <p className="font-sans text-[18px] font-medium leading-[23.46px] tracking-tight text-[#2067CA] text-left">
  ...
  return (
    <div className="mx-auto p-4 font-sans">
      <nav className="md:flex space-x-6 text-sm mb-6 sticky top-[-1px] bg-white z-10 w-full h-[89px] items-center ">
        <div className="item flex space-x-6 h-[40px] pl-32  pl-none">
          <a
            className={`custom-link text-blue-600 ${
              index === 0
                ? "border-b-[3px] border-blue-600 pb-2 borderUnderline"
                : ""
            }`}
          >
      ...
      {tabData.map((data: any) => {
        return (
          <>
            <div className="grid w-full pl-32 pl-none ">
            ...
            <div className="grid gap-6 mb-6">
              <div
                className={`scroll-container ${
                  data?.images.length > 1 ? "w-full flex justify-center" : ""
                }`}
                onMouseEnter={() => {
                  // setIsPlaying(true);
                }}
              >
                <div
                  className={`scroll-content images objectFit ${
                    data?.images.length > 1 ? "play-animation" : "w-full"
                  }`}
                >
                  {data?.images.length > 1 ? (
                    data?.images?.map((src: any, key: any) => (
                      <ImageCard
                        imageUrl={`${src}`}
                    ...
                    <>
                      <div className="w-full objectFit pl-32 ">
                        <ImageCard
                          imageUrl={`${data?.images[0]}`}
                          altText={`Sample Image`}
                          style={{
                            width: data?.images.length > 1 ? "820px" : "90%",
                            height: "400px",
                            borderRadius: "8px",
                          }}
                        />
            ...
            <section className="relative  blade-bottom-padding">
              <div className="flex relative w-container pl-20 pl-none">
                <div className="md:flex-1 space-y-3">
                  <p className="custom-paragraph ">Project</p>
                  <div>
                    <h5
                      className="font-worksansMedium gsap-fade-in"
                      dangerouslySetInnerHTML={{ __html: data.project }}
                    />
                  </div>
                </div>
                        ...
                        {data?.locationArray?.map((text: any, index: any) => (
                          <h6
                            ...
                            className={`gsap-fade-in ${
                              index < data.locationArray.length - 1
                                ? "border-b-2 border-black"
                                : ""
                            } py-[20px]`} // Apply border-b class conditionally
                          >
                            {text}
                          </h6>
                        ))}
